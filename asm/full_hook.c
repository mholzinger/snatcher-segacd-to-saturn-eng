 ...
 * small for a key). build_engine marks those records [0x06][index]; decode() walks to the
 * index-th 1-byte-encoded string in this packed blob and decodes it. A blob (not a char*
 * array) saves the 132-byte pointer table so the payload stays under the paging scratch.
 * Generated by build_engine (order == the build's index) into tok_en.h. */
static const char tok_en_blob[] =
#include "tok_en.h"
;
...
char __attribute__((section(".text.decode"))) *decode(char *p)
{
    ...
    if ((u8)p[0] == 0x06) {
        const char *s = tok_en_blob;
        int n = (u8)p[1];
        while (n-- > 0) { while (*s) s++; s++; }     /* skip to the index-th string */
        p = (char *)s;
    }
    char *start = p;
    int is_dlg = 0;
